...
/** Plural dimension columns (arrays); singular legacy keys merged in detailRowsMerged. */
...
export default function HoqScraperPage() {
  ...
  const [hoqBackendCheck, setHoqBackendCheck] = useState<"idle" | "ok" | "missing" | "error">("idle");

  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const s = await hoqPingStatus();
      if (!cancelled) setHoqBackendCheck(s === "ok" ? "ok" : s === "missing" ? "missing" : "error");
    })();
    return () => {
      cancelled = true;
    };
  ...
  /** Internal m² from detail scrape (property page DOM); total_sqm same — listing wins ref/locality/image only. */
  const detailRowsMerged = useMemo(() => {
    return detailRows.map((d) => {
      ...
      const detailDesc = d.description;
      delete merged.description;
      if (detailDesc != null && String(detailDesc).trim() !== "") merged.description = detailDesc;
      const detailTotal = d.total_sqm;
      delete merged.total_sqm;
      if (detailTotal != null && String(detailTotal).trim() !== "") merged.total_sqm = detailTotal;
      const detailInt = d.internal_sqm;
      delete merged.internal_sqm;
      if (detailInt != null && String(detailInt).trim() !== "") merged.internal_sqm = detailInt;
  ...
  /** After detail scrape, show property-page internal m² in the listings table for that ref. */
  const detailInternalByRef = useMemo(() => {
    const m = new Map<string, unknown>();
    detailRows.forEach((r) => {
      const ref = String(r.reference ?? "");
      const iq = r.internal_sqm;
      if (ref && iq != null && String(iq).trim() !== "") m.set(ref, iq);
    });
    return m;
  }, [detailRows]);
      ...
      <div className="mx-auto max-w-[1400px]">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold text-white md:text-3xl">Homes of Quality — Scraper</h1>
            <p className="mt-1 max-w-2xl text-sm text-slate-400">
              List page: latest properties · Detail: listing-page with reference. Uses Playwright + extraction on the server.
            </p>
          </div>
        </div>

        {hoqBackendCheck === "missing" && (
          <div className="mb-6 rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
            <strong className="font-semibold">Backend needs a restart.</strong>{" "}
            <span className="text-amber-100/90">{HOQ_RESTART_HINT}</span>
          </div>
        )}
        {hoqBackendCheck === "error" && (
          <div className="mb-6 rounded-lg border border-red-500/35 bg-red-500/10 px-4 py-3 text-sm text-red-200">
            Could not reach <code className="rounded bg-black/30 px-1">/api/workbench/hoq/ping</code>. Ensure FastAPI
        ...
        <section className="mb-12 rounded-xl border border-white/10 bg-[var(--bg-card)] p-5 md:p-6">
          <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex-1 space-y-2">
              <h2 className="text-lg font-semibold text-white">Property listings</h2>
          ...
          >
            <table className="w-full min-w-[1280px] border-collapse text-left text-sm">
              ...
              <tbody>
                {displayedListings.map((row, idx) => {
                  ...
                  const intOv = ref ? detailInternalByRef.get(ref) : undefined;
                  const listRow =
                    intOv != null && String(intOv).trim() !== ""
                      ? ({ ...row, internal_sqm: intOv } as Record<string, unknown>)
                      : row;
                  return (
                    <tr key={ref || `row-${idx}`} className="border-b border-white/5 bg-[var(--bg-row-alt)]/40">
                      ...
                      <HoqListingRowCells row={listRow} onPickImage={(u) => setImgModal(u)} />
                    </tr>
                  );
                })}
              </tbody>
            ...
            {!listings.length && !listLoading && (
              <p className="p-6 text-center text-sm text-slate-500">Load listings to populate the table.</p>
            )}
...
function DetailExtraCell({ k, row }: { k: string; row: Record<string, unknown> }) {
  const v = row[k];
